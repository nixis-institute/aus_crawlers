...
class ForeverNew:
    ...
    def crawl_detail(self,url) -> dict or str:
        try:
            ...
            imgs = "|".join([i.replace('\\','') for i in set(re.findall('"img":"(.*?)"',r.text))])
            return {"imgs":imgs}
    ...
    def crawl_link(self,query,row) -> None:
        ...
        query = query.strip()
        url = 'https://www.forevernew.co.in/catalogsearch/result/index/?p={}&q={}'
        ...
        # try:
        total_count = int(tree.xpath('//span[@class="toolbar-number"]//text()')[-1])
        ...
        for i in tqdm(range(1,total_page+1)):
            ...
            products = tree.xpath('//a[@class="product-item-link"]/@href')
            for product in tqdm(products):
                # try:
                    unique_id = product.split('-')[-1].replace('.html','')
                    product_url = product
