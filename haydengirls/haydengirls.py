...
class HydenGirls:
    ...
    def crawl_detail(self,url) -> dict or str:
        try:
            ...
            imgs = "|".join(["https:"+i for i in set(tree.xpath('//div[@class="product product-default"]//img[@class="lazyload"]/@src'))])
            _id = re.search('"productId":(.*?),',r.text).group(1)
            return {"imgs":imgs,"id":_id}
    ...
    def crawl_link(self,query,row) -> None:
        ...
        query = query.strip()
        url = 'https://www.haydengirls.com/search?page={}&q={}&type=product'
        
        r = self.s.get(url.format(1,query))
        tree = html.fromstring(r.text)
        try:
            total_count = int("".join(tree.xpath('//p[@class="page-total"]//text()')).strip().split('    ')[-1])
            ...
            for i in tqdm(range(1,total_page+1)):
                ...
                tree = html.fromstring(r.text)
                products = tree.xpath('//a[@class="product-grid-image"]')
                for product in tqdm(products):

                    product_url = "https://www.haydengirls.com"+ "".join(product.xpath('./@href'))
