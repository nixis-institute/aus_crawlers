...
def copy_folder(source,destination,num):
    ...
    try:
        prd_name = destination['prd_name'].split('/products/')[1]+f'_{num}'
    except:
        prd_name = f'{num}'
    try:
        os.mkdir(retailer)
    ...
    for g in glob(path_to_img+"/*"):
        pr_name = source['prd_name'].split('/products/')[1]
        if(pr_name in g):
            download_img_path = g
...
def _download(row,num):
    try:
        ...
        if(img_download is None):
            for i in images:
                if(i):
                    img_path = f'{retailer}/{path}/{search_term}/{prd_name}'
                    print(f"\n{img_path}")
...
df = pd.read_csv('fashionnova_imgs.csv',index_col=False)
