...
class MarksAndSpencer:
    ...
    def crawl_detail(self,url) -> dict or str:
        try:
            ...
            imgs = "|".join([i+'?$PDP_IMAGEGRID_1_LG$' for i in re.findall('"ImageObject", "url": "(.*?)"',r.text)])
    ...
    def crawl_link(self,query,row) -> None:
        ...
        query = query.strip()
        url = 'https://www.marksandspencer.com/MSFindItemsByKeyword?searchTerm={}&page={}'
        ...
        # try:
        total_count = int(re.search('num_results = "(.*?)"',r.text).group(1))
        ...
        for i in tqdm(range(1,total_page+1)):
            ...
            for product in tqdm(products):
                # try:
                    product_url = "https://www.marksandspencer.com"+product
