...
class Autographfashion:
    def __init__(self) -> None:
        username = 'root'
        password = 'nixis123'
        database_name = 'crawling'
        self.retailer = 'autographfashion'
    ...
    def crawl_detail(self,url) -> dict or str:
        try:
            ...
            imgs = "|".join(tree.xpath('//div[@class="product-image-gallery"]//img/@data-zoom-image'))
            sku = "".join(tree.xpath('//input[@class="cart-productid"]/@value'))
    ...
    def crawl_link(self,query,row) -> None:
        ...
        query = query.strip()
        url = 'https://www.autographfashion.com.au/ProductList/Search?FF=&P.StartPage={}&P.LoadToPage=1&Query={}&sorting=Relevance'
        ...
        # try:
        total_count = int("".join(tree.xpath('//meta[@itemprop="numberOfItems"]/@content')))
        ...
        #     tree = html.fromstring(r.text)
        #     products = tree.xpath('//a[@class="product__link"]/@href')
        for product in tqdm(products):
            # try:
                product_url = "https://www.autographfashion.com.au"+product
