...
class Dissh_AU:
    ...
    def crawl_detail(self,url) -> dict or str:
        try:
            ...
            imgs = "|".join(["https:"+i for i in tree.xpath('//div[@class="MagicToolboxSelectorsContainer no-magic-scroll"]//a/@data-image')])
            return {"imgs":imgs}
    ...
    def crawl_link(self,query,row) -> None:
        ...
        query = query.strip()
        api_url = "https://r8gkme.a.searchspring.io/api/search/search.json?ajaxCatalog=v3&resultsFormat=native&siteId=r8gkme&domain=https://dissh.com.au/search?view=shop&q={}&q={}&page={}&userId=V3-4BD97412-C82A-4DA8-9114-F1A667FED5F0"
        
        r = self.s.get(api_url.format(query,query,1))
        js = r.json()
        total_count = js['pagination']['totalResults']
        total_page = js['pagination']['totalPages']
        print(query,total_count)
        for i in tqdm(range(1,total_page+1)):
            r = self.s.get(api_url.format(query,query,i))
            js = r.json()
            for product in tqdm(js['results']):
                unique_id = product['id']
                product_url = "https://dissh.com.au/products/"+product['handle'] 
